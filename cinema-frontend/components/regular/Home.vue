...
onMounted(async () => {
  ...
  try {
    ...
    featuredMovies.value = data.movies.slice(5, 10)
  } catch (err) {
    error.value = err;
  } finally {
    loading.value = false;
...
<template>
    <div class="min-h-screen ">
      <!-- Hero Section -->
      <div class="relative bg-gradient-to-r from-teal-500 to-indigo-600  py-20 px-6" :class="selectClass">
        <div class="max-w-6xl mx-auto text-center">
          <h1 class="text-4xl font-bold mb-4 mt-10">
            Welcome to the MovieZone 🎥
          </h1>
          <p class="text-lg mb-6">
            Explore your favorite movies, bookmark them, and book tickets in just a few clicks.
          </p>
          <NuxtLink
            to="/user/Movielist"
      ...
      <section class="py-10 ">
        <div class="max-w-6xl mx-auto px-6">
          ...
          <div v-if="loading" class="text-center text-gray-600">Loading movies...</div>
          <div v-else-if="error" class="text-center text-red-600">Error loading movies: {{ error.message }}</div>
          <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            >
              ...
              <div class="p-4">
               ...
               <button @click="ViewDetail(movie.movie_id)" class="mt-4 inline-block text-blue-600 font-semibold hover:underline">
                view details
               </button>
              </div>
            </div>
      ...
      <section class="py-10 ">
        <div class="max-w-6xl mx-auto px-6">
          ...
          <div v-if="loading" class="text-center text-gray-600">Loading featured movies...</div>
          <div v-else-if="error" class="text-center text-red-600">Error loading movies: {{ error.message }}</div>
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <div class="p-4">
                ...
                <button @click="BookMovies(featured.movie_id)" class="mt-4 inline-block text-blue-600 font-semibold hover:underline">
                    Book now
